...
## Your job
1. Read the project (README, package.json, vite.config.*) to understand special requirements
...
- **Port**: Check if `vite.config.*` has `server.port` hardcoded — update to {{PORT}} if so
- **strictPort**: The manifest uses `--port $PORT --host` flags; ensure `server.strictPort = true` in vite.config
- **Framework**: Identify the UI framework (React, Vue, Svelte, etc.) from dependencies

## CRITICAL RULES
- The app MUST bind to port {{PORT}} with strictPort enabled
