...
## Your job
1. Read the project (README, requirements.txt, manage.py, settings module) to understand special requirements
...
- What is missing (e.g. "Python 3.11+ is required but python3 is not found")
...
- **Dependencies**: `pip install -r requirements.txt` (or Pipfile/pyproject.toml) — inside the venv only
- **Environment**: Copy `.env.example` to `.env`, set PORT={{PORT}}, ALLOWED_HOSTS=localhost,127.0.0.1,0.0.0.0, DEBUG=True
- **Database**: Only run `python manage.py migrate` if the database server is already running. If not, report it.
- **PostgreSQL**: If psycopg2 in requirements, check if `psql` is available and the server is running. If not, tell the user.
- **Static files**: Run `python manage.py collectstatic --noinput` (ignore errors)
- **Binding**: The dev command uses `0.0.0.0:$PORT` to bind to all interfaces
