...
## Your job
1. Read the project (README, requirements.txt, app entry point) to understand special requirements
...
5. Set up environment (.env, .flaskenv)
...
- **FLASK_APP**: Set in `.env` or `.flaskenv` — common entry points: `app.py`, `wsgi.py`, `run.py`
- **Environment**: Copy `.env.example` to `.env`, set FLASK_ENV=development
- **Database**: If Flask-Migrate is used, only run `flask db upgrade` if the database is already running
- **PostgreSQL**: If psycopg2 in requirements, check if the server is running. If not, tell the user.
- **Binding**: Uses `--host 0.0.0.0 --port $PORT` to bind to all interfaces
- **Gunicorn**: If the project uses gunicorn, consider updating the manifest dev command
