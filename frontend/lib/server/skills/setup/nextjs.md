...
## Your job
1. Read the project (README, package.json, next.config.*) to understand special requirements
...
- What is missing (e.g. "Node 20+ is required but you have Node 16")
...
- **Environment**: Copy `.env.example` or `.env.local.example` to `.env.local`
- **Port**: Both PORT env var and `-p {{PORT}}` flag are used (Next.js needs both)
- **Prisma**: If `prisma/schema.prisma` exists, run `npx prisma migrate deploy` or `npx prisma db push` — but only if the database is running
- **Drizzle**: If `drizzle/` dir exists, run `npx drizzle-kit push` — but only if the database is running
